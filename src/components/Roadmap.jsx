...
export function RoadmapTrack({ phases, startDate, hexSize = 26 }) {
  const { nodes, progress } = computeRoadmap(phases, startDate);
  if (!nodes.length) return null;
  const half = hexSize / 2 + 1;

  return (
    <div style={{ position: 'relative', height: hexSize + 42, margin: '2px 2px 0' }}>
      ...
      {nodes.map((p) => {
        ...
        // Keep edge labels on-canvas: left-align the first-ish, right-align the
        // last-ish, centre the rest.
        const align = p.endFrac > 0.85 ? 'right' : p.endFrac < 0.15 ? 'left' : 'center';
        const labelTransform = align === 'center' ? 'translateX(-50%)' : align === 'right' ? 'translateX(-100%)' : 'none';
        const labelLeft = align === 'left' ? `calc(${left} - ${half}px)` : left;
        ...
        return (
          <React.Fragment key={p.id}>
            ...
            <div style={{ position: 'absolute', left: labelLeft, top: hexSize + 6, transform: labelTransform, width: 66, textAlign: align }}>
              <div className="mono" style={{ fontSize: 8, letterSpacing: '0.1em', fontWeight: 700, color: p.done || p.current ? 'var(--accent)' : 'var(--text-3)' }}>P{p.idx + 1}</div>
              <div style={{ fontSize: 9, marginTop: 1, lineHeight: 1.15, fontFamily: '"JetBrains Mono", monospace', color: p.current ? 'var(--text)' : p.done ? 'var(--text-2)' : 'var(--text-3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</div>
            </div>
          </React.Fragment>
        );
      })}
